...
export class ClientPerformanceObserver {
  ...
  public start(callback?: (payload: { cda: string, cma: string }) => void): void {
    ...
    this.updater = setInterval(() => {
      ...
      this.cmaPerformanceMessage = `average ${OutputFormatter.formatNumber(cmaAverageCallsPerSecond + '/s')}, current ${OutputFormatter.formatNumber(cmaCallsLastSecond + '/s')}`
      this.cdaPerformanceMessage = `average ${OutputFormatter.formatNumber(cdaAverageCallsPerSecond + '/s')}, current ${OutputFormatter.formatNumber(cdaCallsLastSecond + '/s')}`
